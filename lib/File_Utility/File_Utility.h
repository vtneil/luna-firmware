...
public:
  ...
  void find_file_name_one(const char *prefix, const char *extension = "csv") {
    m_filename = find_file_name(prefix, extension);
  }

  String find_file_name(const char *prefix, const char *extension = "csv") {
    String   ret;
    uint32_t file_idx = 1;
    do {
      ret = "";
      ret << prefix << file_idx++ << "." << extension;
    } while (m_sd.exists(ret.c_str()));
    return ret;
  }
